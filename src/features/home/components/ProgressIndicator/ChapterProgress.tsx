...
export function ChapterProgress() {
  ...
  return (
    <>
      {/* ── Top Progress Bar ── */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-[3px] bg-primary z-[100] origin-left shadow-[0_0_8px_rgba(var(--color-brand),0.3)]"
        style={{
          scaleX,
          ...
          {CHAPTERS.map((chapter, index) => {
            ...
            return (
              ...
              >
                {/* Chapter Label */}
                <span
                  className={`text-[10px] translate-x-0 font-mono font-bold tracking-[0.2em] uppercase transition-all duration-500 transform ${isActive
                ...
                <div className="relative">
                  <motion.div
                    ...
                    animate={{
                      ...
                      backgroundColor: isActive ? "hsl(var(--color-brand))" : "rgba(255, 255, 255, 0.15)",
                    }}
                    className={`w-1.5 h-1.5 rounded-full transition-colors duration-500 ${isActive ? "shadow-[0_0_10px_rgba(var(--color-brand),0.5)]" : "group-hover:bg-muted-foreground/50"
                      }`}
                  />
