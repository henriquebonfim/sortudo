import { useNotableDraw } from "@/application/selectors";
...
export function CaseStudy({ title }: { title?: string }) {
  ...
  const breakdownData = [
    { name: "Prêmio Bruto", value: REVENUE_ALLOCATION.PRIZE_POOL * 100, color: "hsl(142, 71%, 45%)" },
    { name: "Seguridade Social", value: REVENUE_ALLOCATION.SOCIAL_SECURITY * 100, color: "hsl(217, 91%, 60%)" },
    { name: "Custeio Operacional", value: REVENUE_ALLOCATION.OPERATIONAL_CUSTEIO * 100, color: "hsl(215, 16%, 47%)" },
    { name: "Comissão dos Lotéricos", value: REVENUE_ALLOCATION.OPERATIONAL_COMMISSION * 100, color: "hsl(215, 20%, 55%)" },
    { name: "Seguridade Pública (FNSP)", value: REVENUE_ALLOCATION.PUBLIC_SECURITY * 100, color: "hsl(0, 84%, 60%)" },
    { name: "Fundo Penitenciário (FUNPEN)", value: REVENUE_ALLOCATION.PENITENTIARY * 100, color: "hsl(280, 65%, 60%)" },
    { name: "Fundo de Cultura (FNC)", value: REVENUE_ALLOCATION.CULTURE * 100, color: "hsl(190, 90%, 50%)" },
    { name: "Ministério do Esporte", value: REVENUE_ALLOCATION.SPORT * 100, color: "hsl(150, 60%, 50%)" },
    { name: "Comitê Olímpico (COB)", value: REVENUE_ALLOCATION.COB * 100, color: "hsl(38, 92%, 50%)" },
    { name: "Secretarias de Esporte", value: REVENUE_ALLOCATION.STATE_SPORTS * 100, color: "hsl(38, 80%, 60%)" },
    { name: "Comitê Paralímpico (CPB)", value: REVENUE_ALLOCATION.CPB * 100, color: "hsl(38, 70%, 55%)" },
    { name: "FDL", value: REVENUE_ALLOCATION.OPERATIONAL_FDL * 100, color: "hsl(215, 25%, 65%)" },
    { name: "Comitê Brasileiro de Clubes", value: REVENUE_ALLOCATION.CBC * 100, color: "hsl(38, 60%, 45%)" },
    { name: "Desporto Escolar (CBDE)", value: REVENUE_ALLOCATION.CBDE * 100, color: "hsl(38, 50%, 40%)" },
    { name: "Desporto Universitário (CBDU)", value: REVENUE_ALLOCATION.CBDU * 100, color: "hsl(38, 40%, 35%)" },
    { name: "Clubes Paralímpicos", value: REVENUE_ALLOCATION.CBCP * 100, color: "hsl(38, 30%, 30%)" },
  ...
  return (
    <motion.div
      ...
      style={{ boxShadow: "0 0 0 1px hsl(var(--primary)/0.08), 0 32px 64px -12px hsl(var(--primary)/0.12)" }}
    >
      {/* ── Header ─────────────────────────────────────────────────── */}
      <div className="px-6 md:px-10 pt-8 pb-6 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <span className="inline-flex items-center gap-1.5 text-[10px] font-mono font-semibold text-primary uppercase tracking-[0.2em] bg-primary/10 border border-primary/20 rounded-full px-3 py-1 w-fit">
            Estudo de Caso · Concurso #{notableDraw.id}
          </span>
        ...
        <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 px-4 py-2.5 rounded-2xl self-start sm:self-auto">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] text-muted-foreground font-mono leading-none mb-0.5">Data do Sorteio</span>
            <span className="text-base font-bold font-mono text-primary leading-none">
              {new Date(notableDraw.date).toLocaleDateString("pt-BR")}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-border">
        ...
        <div className="px-5 md:px-8 py-5 flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground font-medium mb-1 whitespace-nowrap">Arrecadação Total</p>
            <p className="text-sm font-bold font-mono text-foreground leading-tight break-all">{fmt(revenue)}</p>
          </div>
        </div>
        ...
        <div className="px-5 md:px-8 py-5 flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-bold font-mono text-red-500 leading-tight break-all">{fmt(socialRepasses)}</p>
          </div>
        </div>
        ...
        <div className="px-5 md:px-8 py-5 flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-bold font-mono text-green-500 leading-tight break-all">{fmt(prizeBruto)}</p>
          </div>
        </div>
      </div>
        ...
        <div className=" lg:col-span-2 px-6 md:px-10 py-8 flex flex-col items-center justify-center">
          ...
          <div className="relative w-[220px] h-[220px]">
                ...
                <Tooltip
                  ...
                  formatter={(v: number, name: string) => [
                    <div key="tt" className="flex flex-col items-end gap-0.5 ">
                      <span className="font-bold">{v.toFixed(2)}% - {name}: </span>
            ...
            <div className="absolute -z-10 inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest leading-none mb-1">Prêmio</span>
              <span className="text-2xl font-bold font-mono text-green-500 leading-tight">{prizePct}%</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-3 px-6 md:px-10 py-8">
                ...
                >
                  <div className="flex justify-between items-end mb-1.5 gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[10px] font-mono text-muted-foreground">{d.value.toFixed(2)}%</span>
                      <span className="text-xs font-mono font-bold text-foreground">{fmt(revenue * (d.value / 100))}</span>
                    </div>
                  </div>
          ...
          >
            {isOpen ? (
              ...
              <>Mostrar todos os {breakdownData.length} destinos <ChevronDown className="w-3.5 h-3.5" /></>
            )}
          </motion.button>
        </div>
      </div>
