  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={`text-[0.5rem] uppercase tracking-widest font-bold ${isHeads ? "text-amber-500/70" : "text-slate-500/70"
          }`}>
          {isHeads ? "cara" : "coroa"}
        </span>
      </div>
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground leading-relaxed">
        ...
        cair no mesmo lado. Cada jogada tem{" "}
        <span className="font-mono text-primary font-semibold">{prob}</span> de chance — independente das anteriores.
      </p>
            ...
            <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 px-4 py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Depois de <span className="text-amber-400 font-mono">{attempts}</span> tentativas você conseguiu acertar <span className="text-amber-400 font-mono">{wins}</span> {wins > 1 ? 'vezes' : 'vez'}!
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  A próxima jogada ainda tem <span className="text-amber-400 font-mono">{prob}</span> de chance.
                </p>
              </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl bg-muted/40 border border-border px-2 py-3">
          ...
          <div className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Probabilidade</div>
        </div>
        <div className="rounded-xl bg-muted/40 border border-border px-2 py-3">
          <div className="text-base font-bold font-mono text-foreground tabular-nums">{wins}</div>
          <div className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Acertos</div>
        </div>
        <div className={`rounded-xl border px-2 py-3 transition-colors ${allSame && attempts > 0
          ? "bg-amber-500/10 border-amber-500/30"
          : "bg-muted/40 border-border"
          }`}>
          <div className={`text-base font-bold font-mono tabular-nums ${allSame && attempts > 0 ? "text-amber-400" : "text-muted-foreground"
            }`}>
            {allSame && attempts > 0 ? "✓" : "—"}
          </div>
          <div className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Agora</div>
        </div>
      </div>
