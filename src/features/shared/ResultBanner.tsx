import { Draw, SearchResult } from '@/domain/lottery/data/draw';
...
function JackpotDetails({ draw }: { draw: Draw }) {
  return (
    <motion.div
...
function MatchGroup({ group, combination }: { group: { label: string, hits: number, data: Draw[], color: string, bg: string, border: string }, combination: number[] }) {
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-white/5">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-widest border-b border-white/5">Jogo</th>
                    ...
                    {group.data.map((d: Draw) => (
                      <tr key={d.id} className="hover:bg-white/5 transition-all duration-150 group/row">
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground font-medium flex flex-row gap-1.5 items-center">
                          {d.numbers.map((n) => (
                            <MiniBall key={n} number={n} size="sm" dimmed={!combination.includes(n)} />
                          ))}
                        </td>
