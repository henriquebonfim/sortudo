...
const CustomTooltip = memo(function CustomTooltip({
  ...
  return (
    <div className="glass-card border border-primary/20 p-3 text-xs font-mono space-y-1">
      ...
      <p className="text-muted-foreground/70 border-t border-white/10 pt-1">
        Total: {total}
      </p>
    </div>
  );
...
const AverageIndicator = memo(function AverageIndicator({ avgPct }: AverageIndicatorProps) {
  return (
    <div
      ...
      style={{
        backgroundColor: `${CHART_COLORS.VIOLET}1a`,
        borderColor: `${CHART_COLORS.VIOLET}33`,
      }}
    >
      <span
        className="text-2xl font-display font-bold"
        style={{ color: CHART_COLORS.VIOLET }}
      >
        {avgPct}%
      </span>
...
export default function AccumulationTrendChart() {
  const stats = useLotteryStore((state) => state.stats);
  const data = stats?.accumulationTrend;
