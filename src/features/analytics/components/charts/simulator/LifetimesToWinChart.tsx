  ...
  const chartData = useMemo(() => {
    ...
    return lifetimesToChart.map((lifetimes) => {
      ...
      return {
        lifetimes: lifetimes,
        displayLifetimes:
          lifetimes > 100 ? `${(lifetimes / 1000).toFixed(1)}k Vidas` : `${lifetimes} Vidas`,
    ...
    <div className="space-y-4">
      <div className="glass-card p-6 border-l-4 border-l-red-900/50">
        <h3 className="text-xl font-bold mb-2 text-white">
          O Risco da Ruína (100 Bilhetes/Semana por 80 Anos)
        </h3>
        <p className="text-sm text-slate-300 mb-6 max-w-2xl leading-relaxed">
          É fácil dizer "eu nunca vou ganhar", mas é difícil visualizar matematicamente. Suponha que
          você compre 100 bilhetes todas as semanas, por 80 anos seguidos (custando $832 mil).
          <br />
          <br />
          Mesmo que você jogasse por 3.000 vidas contínuas, ainda haveria uma chance estatística de
          morrer sem nunca ter ganhado o prêmio.{' '}
          <strong>O "Risco da Ruína" nunca desaparece de verdade.</strong>
        </p>
          ...
          <AreaChart data={chartData} margin={{ left: 10, right: 30, top: 40, bottom: 40 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => {
                if (name === 'winProb')
                  return [`${value}% Chance`, 'Probabilidade de um único prêmio'];
            ...
            <ReferenceLine
              ...
              strokeDasharray="5 5"
              label={{
                value: 'Limite de 50/50 (Cara ou Coroa)',
                fill: '#EAB308',
