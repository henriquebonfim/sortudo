import { useTopJackpotWinners } from '@/hooks/use-analytics';
import { TopJackpotWinnersDraw } from '@/shared/types';
...
interface TopJackpotWinnersResponse {
    ...
    >
      ...
      <div className="relative z-10 flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isFirst && (
            <span className="text-[9px] px-1.5 py-0.5 rounded font-mono uppercase tracking-wider bg-primary/20 text-primary">
              recorde
            </span>
          )}
        </div>
      ...
      <div className="relative z-10 text-right flex-shrink-0">
        <div className="font-mono text-xs text-foreground">{formatCurrency(draw.prize)}</div>
        <div className="text-[9px] text-muted-foreground">{draw.pctOfTotalWinners}% do total</div>
      </div>
    </motion.div>
  );
});
