import { ANALYSIS_CONFIG, BALLS_PER_DRAW, LOW_HIGH_BOUNDARY, PRIMES } from '@/lib/lottery/constants';
import { Game } from '@/lib/lottery/types';
import { calculatePercentage, sortGamesById, sum } from '@/lib/lottery/utils';
...
export function calculateNumberProfile(games: Game[]) {
  ...
  const overlapHistory: { drawId: number; prevDrawId: number; date: string; numbers: number[]; count: number }[] = [];
