import { MAX_LOTTERY_NUMBER, PRIMES } from '@/lib/lottery/constants';
...
const STRATEGIES: Record<GenerationMode, GeneratorStrategy> = {
  ...
  winners: (ctx) => {
    if (ctx.games.length === 0) return pickRandom(FULL_POOL, 6);
    const game = ctx.games[Math.floor(Math.random() * ctx.games.length)];
    return pickRandom(game.numbers, 6, FULL_POOL);
  },
  '0odds-6evens': () => pickRandom(EVENS_POOL, 6),
  '1odd-5evens': () =>
    [...pickRandom(ODDS_POOL, 1), ...pickRandom(EVENS_POOL, 5)].sort((a, b) => a - b),
  '2odds-4evens': () =>
    [...pickRandom(ODDS_POOL, 2), ...pickRandom(EVENS_POOL, 4)].sort((a, b) => a - b),
  '3odds-3evens': () =>
    [...pickRandom(ODDS_POOL, 3), ...pickRandom(EVENS_POOL, 3)].sort((a, b) => a - b),
  '4odds-2evens': () =>
    [...pickRandom(ODDS_POOL, 4), ...pickRandom(EVENS_POOL, 2)].sort((a, b) => a - b),
  '5odds-1even': () =>
    [...pickRandom(ODDS_POOL, 5), ...pickRandom(EVENS_POOL, 1)].sort((a, b) => a - b),
...
export class NumberGenerator {
  ...
  static generateRandomSequence(count: number): number[] {
    return pickRandom(FULL_POOL, count);
  }
}
